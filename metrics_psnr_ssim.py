...
def evaluate(hr_dir, sr_dir, scale=4):
    psnr_list = []
    ssim_list = []

    hr_files = sorted([f for f in os.listdir(hr_dir) if f.endswith('.png')])
    for hr_file in tqdm(hr_files, desc="Evaluating"):
        sr_file = hr_file.replace('.png', 'x4.png')
        hr_path = os.path.join(hr_dir, hr_file)
        sr_path = os.path.join(sr_dir, sr_file)

        if not os.path.exists(sr_path):
            print(f"Warning: SR image not found for {hr_file}")
            continue

        psnr_val, ssim_val = cal_psnr_ssim(sr_path, hr_path)
        psnr_list.append(psnr_val)
        ssim_list.append(ssim_val)

    print("Average PSNR: {:.4f} dB".format(np.mean(psnr_list)))
    print("Average SSIM: {:.4f}".format(np.mean(ssim_list)))

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--hr', type=str, required=True, help='Path to HR ground truth images')
    parser.add_argument('--sr', type=str, required=True, help='Path to SR images')
    args = parser.parse_args()

    evaluate(args.hr, args.sr)
